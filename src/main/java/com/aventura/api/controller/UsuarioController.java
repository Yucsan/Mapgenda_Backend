...
public class UsuarioController {
	...
	public ResponseEntity<?> loginConGoogle(@RequestBody Map<String, String> body) {
		...
		GoogleIdTokenVerifier verifier = new GoogleIdTokenVerifier.Builder(new NetHttpTransport(), new JacksonFactory())
				.setAudience(Collections
						.singletonList("** google id user ****.apps.googleusercontent.com")) // 👈
																														// importante
				.build();
	...
	public void testPasswordEncoder() {
		...
		boolean ok = passwordEncoder.matches("admin123", hash); //esto es una prueba no te emociones!!!!
	...
	public ResponseEntity<?> eliminarImagen(@RequestHeader("Authorization") String authHeader,
	    ...
	    try {
	        ...
	        System.out.println("🧪 ENV → CLOUDINARY_CLOUD_NAME = " + cloudName);
	        System.out.println("🧪 ENV → CLOUDINARY_API_KEY = " + apiKey);
	        System.out.println("🧪 ENV → CLOUDINARY_API_SECRET = " +
	            (apiSecret != null ? "✔️ loaded" : "❌ null"));
	        ...
	        System.out.println("❌ Error al eliminar imagen de Cloudinary");
